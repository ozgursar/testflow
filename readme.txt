=== TestFlow ===
Contributors: ozgursar
Tags: testing, test team, session, moderator, patch testing
Requires at least: 6.0
Tested up to: 7.0
Requires PHP: 7.4
Stable tag: 1.0.2
License: GPLv2 or later
License URI: https://www.gnu.org/licenses/gpl-2.0.html

Session facilitator for WordPress Test Team members.

== Description ==

TestFlow helps WordPress Test Team moderators facilitate weekly test chat and patch testing scrub sessions.

= Supported session types =

* **Patch Testing Scrub** — weekly 1-hour session where the moderator assigns tickets to contributors for patch testing
* **Test Chat** — weekly 1-hour team chat covering agenda discussions, ecosystem announcements, and open floor

== Installation ==

1. Upload the plugin to `wp-content/plugins/testflow/`, or install it directly from the WordPress admin **Plugins** screen
2. Activate the plugin from the WordPress admin **Plugins** screen
3. Navigate to **TestFlow** in the admin sidebar

== Frequently Asked Questions ==

= Who is this plugin for? =

WordPress Test Team moderators who run Patch Testing Scrub and Test Chat sessions.

== Screenshots ==

1. Patch Testing Scrub
2. Test Chat
3. Overview

== Changelog ==

= 1.0.2 =
* Trac tickets read as #ID in chat - props @huzaifaalmesbah
* Added WordPress Playground blueprint for Live Preview.

= 1.0.1 =
* Added TestFlow link to the admin bar.

= 1.0.0 =
* Initial release.
